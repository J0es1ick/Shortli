...
SQL-миграции находятся в `backend/internal/database/migrations`. Они встроены
в исполняемый файл backend и применяются перед запуском HTTP-сервера.
...
Перед возвратом перенаправления события аналитики записываются в
`shortli-click-spool`. Затем фоновые обработчики переносят их в PostgreSQL.
Необработанные файлы переживают перезапуск контейнера и автоматически
повторяются. Каждое событие имеет уникальный ключ в базе данных, поэтому сбой
между фиксацией транзакции и удалением локального файла не удваивает переход.

При штатном завершении API прекращает принимать запросы, обрабатывает
оставшуюся очередь и сохраняет на диске события, которые не удалось записать.
Они будут обработаны при следующем запуске.
