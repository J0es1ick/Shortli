...
4. При использовании HTTPS задайте `COOKIE_SECURE=true`.
...
Администраторы рассматривают обращения на странице `/stats`. Обращение можно
...
его поддомены нельзя использовать для новых ссылок, пока администратор не
снимет блокировку.

Для создания ссылок, аутентификации и отправки обращений действуют отдельные
ограничения частоты запросов. Валидатор URL также отклоняет учётные данные в
...
Эти же проверки и сборка обоих контейнеров выполняются в GitHub Actions.
...
администратор. Последнего администратора нельзя удалить или лишить прав.
